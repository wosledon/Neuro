...
- 采用扁平化/模块化目录结构：
  - src/
    ...
    - pages/       — 路由页面
    - hooks/       — 自定义 hooks
    - services/    — 与后端通信的 API 封装（可由 OpenAPI/swagger.json 生成客户端作为开发便利，但不是面向最终用户的产品功能）
    - utils/       — 工具函数
...
生成文档的规则（AI 生成项目文档）
- 数据来源优先级（用于生成文档时）：
  1. 代码注释（公共 API 的 XML 注释、JSDoc）
  2. README 和项目文档
  3. 知识库条目（向量检索结果）
  4. OpenAPI/swagger.json（可作为开发时的结构化接口来源，但不应在面向用户的营销页面中作为功能点）
